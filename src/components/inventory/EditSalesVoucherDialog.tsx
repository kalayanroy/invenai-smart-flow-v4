...
import { SalesVoucher, SalesVoucherItem } from '@/hooks/useSalesVouchers';
...
export const EditSalesVoucherDialog = ({ open, onOpenChange, voucher, onVoucherUpdated }: EditSalesVoucherDialogProps) => {
  ...
  const getCalculatedStock = (productId: string) => {
    ...
    // Get sales from vouchers (excluding current voucher being edited)
    const voucherSales = salesVouchers
      .filter(sv => sv.id !== voucher?.id) // Exclude current voucher
      .flatMap(sv => sv.items.filter(item => item.productId === productId));
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    for (const item of validItems) {
      ...
      const originalItem = voucher.items.find(vItem => vItem.productId === item.productId);
      const originalQuantity = originalItem ? originalItem.quantity : 0;
      const quantityIncrease = item.quantity - originalQuantity;
      
      if (quantityIncrease > 0) {
        const availableStock = getCalculatedStock(item.productId);
        if (product && availableStock < quantityIncrease) {
          alert(`Insufficient stock for ${item.productName}. Available: ${availableStock}, Additional needed: ${quantityIncrease}`);
          return;
        }
      }
    }
    ...
    try {
      await onVoucherUpdated(voucher.id, updates);
